...
Advanced Stats via hoopR Python package
Run: streamlit run march_madness_app.py
Install: pip install streamlit hoopR pandas numpy
"""
...
# 1. Data loading
# ─────────────────────────────────────────────────────────────
@st.cache_data(show_spinner="Loading team box scores…")
def load_box_cached(season: int = 2026) -> pd.DataFrame:
    cache_file = f"mbb_box_{season}.pkl"
    if os.path.exists(cache_file):
        return pd.read_pickle(cache_file)
    try:
        from hoopR import load_mbb_team_box  # type: ignore
        box = load_mbb_team_box(seasons=season)
        box.to_pickle(cache_file)
        return box
    except Exception as e:
        st.error(
            f"Could not load hoopR data: {e}\n\n"
            "Make sure hoopR is installed: `pip install hoopR`"
        )
        return pd.DataFrame()
...
def build_advanced_metrics(box: pd.DataFrame) -> pd.DataFrame:
    ...
    return grp[grp["games"] >= 10].reset_index(drop=True)
...
<div class="topbar">
  ...
  <p>2025–26 Men's College Basketball · Advanced Stats · hoopR</p>
</div>
""", unsafe_allow_html=True)
